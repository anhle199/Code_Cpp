...
template <class T>
static Node<T> *makeNode(const T &data);
...
// - Chèn vào vị trí pos.
template <class T>
void insertAtPositon(List<T> &list, const int &pos, const T &data);
...
void insertAt(List<T> &list, const int &pos, const T &data)
{
    ...
    if (pos < 0 || pos > n)
    {
        ...
        insertAtFirst(list.head, list.tail, data);
    else if (pos == n)
        insertAtLast(list.head, list.tail, data);
    else
    { // 0 < pos < n
        Node<T> *p = list.head;
...
void removeAt(List<T> &list, const int &pos)
{
    ...
    if (pos < 0 || pos >= n)
    {
        ...
        removeFirst(list.head, list.tail);
    else if (pos == n - 1)
        removeLast(list.head, list.tail);
    else
    { // 0 < pos < n - 1
        Node<T> *p = list.head;
