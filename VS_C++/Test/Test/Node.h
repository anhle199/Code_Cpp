...
template <class T>
Node<T>* Node<T>::clone(Node<T>*& tree) {
	Node<T>* p = nullptr;
...
template <class T>
void Node<T>::remove(Node<T>* tree, const T& data) {
	if (tree) {
		if (tree->data == data) {
			Node<T>* p = tree;
